 ...
 * @returns Water quality report with metrics and readings
 * 
...
async function handleGenerateWaterQualityReport(
  ...
  try {
    ...
    const summary = {
      ...
      averageTurbidity: devices.reduce((sum, d) => sum + d.metrics.avgTurbidity, 0) / devices.length,
    };
 ...
 * @returns Device status report with health metrics
 * 
 * @throws {HttpsError} internal - Database operation failed
 */
async function handleGenerateDeviceStatusReport(
  ...
  try {
    ...
    for (const deviceDoc of devicesSnapshot.docs) {
      ...
      devices.push({
        ...
        lastSeen: deviceData.lastSeen
          ? new Date(deviceData.lastSeen).toISOString()
          : "Unknown",
        uptime,
      });
    }
    ...
    const healthScore = totalDevices > 0
      ? ((statusCounts.online / totalDevices) * 100).toFixed(1)
      : "0.0";
 ...
 * 
 * @param request - Callable request with optional deviceIds and date range
 * @returns Data summary report with statistics
 * 
 * @throws {HttpsError} internal - Database operation failed
 */
 ...
 * 
 * @param request - Callable request with optional deviceIds and date range
 * @returns Compliance report with regulatory metrics
 * 
 * @throws {HttpsError} internal - Database operation failed
 */
