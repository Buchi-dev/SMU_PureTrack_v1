...
function shouldNotifyUser(
  ...
  if (preferences.devices && preferences.devices.length > 0) {
    const hasMatchingDevice = alerts.some((alert) =>
      preferences.devices.includes(alert.deviceId)
    );
 ...
 * @param preferences - User notification preferences
 * @param alerts - Array of stale alerts
 * @returns Filtered array of alerts matching user preferences
 */
    ...
    try {
      ...
      for (const prefDoc of preferencesSnapshot.docs) {
        ...
        try {
          ...
          emailsSent++;
          logger.info(
            `Sent stale alert email to ${preferences.email} ` +
            `(${filteredAlerts.length} alerts)`
          );
        } catch (error) {
          emailsFailed++;
          logger.error(
            `Failed to send stale alert email to ${preferences.email}:`,
            error
          );
